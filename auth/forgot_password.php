...
function smtp_mailer($email, $subject, $msg)
{
    ...
    $mail->SetFrom(GMAIL_SENDER_EMAIL, "Advita");
...
function sendPasswordResetLink($email, $resetToken, $ip_address)
{
    ...
    $logo = 'https://linksync.free.nf/icons/advaita.png';  // replace with actual link
    $message = '
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="container">
                <div class="logo">
                    <a LinkSync> </a><img src="' . $logo . '" alt="Adaita">
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if the user has already made three requests within the last 10 minutes
    $rate_limit_check_sql = "SELECT COUNT(*) AS request_count FROM password_reset_requests WHERE email = ? AND request_time >= DATE_SUB(NOW(), INTERVAL 10 MINUTE)";
    ...
    if ($rate_limit_row['request_count'] >= 3) {
...
<head>
    ...
    <link rel="stylesheet" href="../css/newforget.css">
</head>
    ...
    <body>
        <div class="home">
            ...
            <div class="container">
                ...
                <p class="login-link"><a href="../">Home</a></p>
            </div>
        </div>
    </body>
